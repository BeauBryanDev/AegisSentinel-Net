...
def _load_session(model_path: Path, model_name: str) -> ort.InferenceSession:
    ...
    if not model_path.exists():
        ...
        raise FileNotFoundError(
            ...
            f"Make sure the .onnx file is present in ml_models/ before building Docker."
        )
 
    session_options = ort.SessionOptions()
    # Enable all graph optimizations for maximum inference speed
...
class ModelRegistry:
    ...
    """
    # The sessions are initialized in the load() method, not in __init__.
    ...
    def warmup(self) -> None:
        ...
        remove the JIT compilation delay on the first real inference request.
